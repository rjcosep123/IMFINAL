        ...
        <form action="{{ route('staff.update', $staff->staff_id) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" id="first_name" name="first_name" value="{{ old('first_name', $staff->first_name) }}" class="w-full border-gray-300 rounded-md p-2" required />
                </div>

                <div>
                    <label for="last_name" class="block text-gray-700 font-semibold text-sm mb-1">Last Name <span class="text-red-500">*</span></label>
                    <input type="text" id="last_name" name="last_name" value="{{ old('last_name', $staff->last_name) }}" class="w-full border-gray-300 rounded-md p-2" required />
                </div>
                ...
                <div>
                    ...
                    <input type="date" id="date_of_birth" name="date_of_birth" value="{{ old('date_of_birth', $staff->date_of_birth) }}" class="w-full border-gray-300 rounded-md p-2" required />
                </div>

                <div>
                    <label for="gender" class="block text-gray-700 font-semibold text-sm mb-1">Gender <span class="text-red-500">*</span></label>
                    <select id="gender" name="gender" class="w-full border-gray-300 rounded-md p-2" required>
                ...
                <div>
                    ...
                    <input type="text" id="national_id" name="national_id" value="{{ old('national_id', $staff->national_id) }}" class="w-full border-gray-300 rounded-md p-2" />
                </div>

                <div>
                    <label for="contact_email" class="block text-gray-700 font-semibold text-sm mb-1">Email <span class="text-red-500">*</span></label>
                    <input type="email" id="contact_email" name="contact_email" value="{{ old('contact_email', $staff->contact_email) }}" class="w-full border-gray-300 rounded-md p-2" required />
                </div>

                <div>
                    <label for="phone_number" class="block text-gray-700 font-semibold text-sm mb-1">Phone Number</label>
                    <input type="text" id="phone_number" name="phone_number" value="{{ old('phone_number', $staff->phone_number) }}" class="w-full border-gray-300 rounded-md p-2" />
                </div>

                <div>
                    <label for="address" class="block text-gray-700 font-semibold text-sm mb-1">Address</label>
                    <input type="text" id="address" name="address" value="{{ old('address', $staff->address) }}" class="w-full border-gray-300 rounded-md p-2" />
                </div>

                <div>
                    <label for="job_title" class="block text-gray-700 font-semibold text-sm mb-1">Job Title <span class="text-red-500">*</span></label>
                    <input type="text" id="job_title" name="job_title" value="{{ old('job_title', $staff->job_title) }}" class="w-full border-gray-300 rounded-md p-2" required />
                </div>

                <div>
                    <label for="department" class="block text-gray-700 font-semibold text-sm mb-1">Department</label>
                    <input type="text" id="department" name="department" value="{{ old('department', $staff->department) }}" class="w-full border-gray-300 rounded-md p-2" />
                </div>

                <div>
                    <label for="hire_date" class="block text-gray-700 font-semibold text-sm mb-1">Hire Date <span class="text-red-500">*</span></label>
                    <input type="date" id="hire_date" name="hire_date" value="{{ old('hire_date', $staff->hire_date) }}" class="w-full border-gray-300 rounded-md p-2" required />
                </div>
                ...
                <div>
                    ...
                    <select id="employment_type" name="employment_type" class="w-full border-gray-300 rounded-md p-2" required>
                        <option value="">Select...</option>
                ...
                <!-- ✅ Ward Allocation Dropdown -->
                <div>
                    <label for="ward_id" class="block text-gray-700 font-semibold text-sm mb-1">Ward Allocation</label>
                    <select id="ward_id" name="ward_id" class="w-full border-gray-300 rounded-md p-2">
                        <option value="">Select a ward...</option>
                        @foreach ($wards as $ward)
                            <option value="{{ $ward->id }}" {{ old('ward_id', $staff->ward_id) == $ward->id ? 'selected' : '' }}>
                                {{ $ward->ward_name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div>
                    ...
                    <textarea id="qualifications" name="qualifications" rows="4" class="w-full border-gray-300 rounded-md p-2">{{ old('qualifications', $staff->qualifications) }}</textarea>
                </div>

                <div>
                    <label for="work_experience" class="block text-gray-700 font-semibold text-sm mb-1">Previous Work Experience</label>
                    <textarea id="work_experience" name="work_experience" rows="4" class="w-full border-gray-300 rounded-md p-2">{{ old('work_experience', $staff->work_experience) }}</textarea>
                </div>
            </div>

            <div class="mt-8 flex justify-end space-x-4">
                <a href="{{ route('staff.index') }}" class="px-6 py-3 border border-gray-300 text-base font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">Cancel</a>
                <button type="submit" class="px-6 py-3 text-base font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">Update Staff</button>
            </div>
        </form>
    </div>
</div>
